			...
			switch ($this->subModule) {
				case "tarty" :
					...
					if ($tab ["dostepna"] == "on")
						...
						/*
					 * if($_FILES['file']['error']==0){ $foto = new Photo($_FILES['file'],"data/tarty/"); $foto->createMiniFixed("200px"); $foto->resizeFixed("400px"); //!!!!!!!!!!!!! $tab_tarty['zdjecie_url'] = $foto->getName(); }else { $tab_tarty['zdjecie_url']=""; }
					 */
				...
				case "pozostale" :
					...
					/*
					 * if($_FILES['file']['error']==0){ $foto = new Photo($_FILES['file'],"data/pozostale/"); $foto->createMiniFixed("200px"); $tab['zdjecie_url'] = $foto->getName(); }else { $tab['zdjecie_url']=""; }
					 */
					// koniec dodawania zdjęcia
					if (isset ( $tab ['file_up'] )) {
		...
		if ($this->subModule != "") {
			...
			switch ($type) {
				case "tarty" :
					...
					echo "<a href='/" . $foto->getFullPath () . "' class='galeria'>" . "<img src='/" . $foto->getFullPathMini () . "' /></a>" . "<input type='hidden' name='file_up[]' value='" . $foto->getFullPath () . "' />" . "<input id='tarty_' type='button' style='margin-top: 5px;' class='remover button' value='Usuń' />";
					die ();
					break;
				case "skladniki" :
					break;
